...
{
	Vector2f mins = Vector2f( m_position.x - m_cosmeticRadius, m_position.y - m_cosmeticRadius );
	Vector2f maxs = Vector2f( m_position.x + m_cosmeticRadius, m_position.y + m_cosmeticRadius );
	m_renderBounds = AABB2f( mins, maxs );
...
void Entity2D::Update( float deltaSeconds )
{
	...
	m_renderBounds.mins = Vector2f( m_position.x - m_cosmeticRadius, m_position.y - m_cosmeticRadius );
	m_renderBounds.maxs = Vector2f( m_position.x + m_cosmeticRadius, m_position.y + m_cosmeticRadius );
...
void Entity2D::Render()
{
	...
	g_theRenderer->DrawPolygon( VertexGroupingRule::AS_LINE_LOOP, m_position, m_cosmeticRadius, DEFAULT_ENTITY_NUM_SIDES, 0.f );
// 
...
//--------------------------------------------------------------------------------------------------------------
Vector2f Entity2D::GetLocalTipPosition() const
{
...
//--------------------------------------------------------------------------------------------------------------
Vector2f Entity2D::GetDirection() const
{
	Vector2f directionVector = Vector2f( CosDegrees( m_orientation ), SinDegrees( m_orientation ) );
