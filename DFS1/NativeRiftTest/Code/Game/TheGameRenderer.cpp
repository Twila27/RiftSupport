...
void TheGame::Render2D()
{
	...
	g_theRenderer->DrawTextProportional2D( g_theRenderer->GetScreenCenter(), "TEST" );

	AABB2f bounds;
	bounds.maxs = Vector2f( 818.f, 422.f );
	bounds.mins = Vector2f( 803.f, 423.f );
	g_theRenderer->DrawAABB( VertexGroupingRule::AS_TRIANGLES, bounds, Rgba::RED, 5.f );
}
...
void TheGame::RenderReticle()
{
	...
	g_theRenderer->SetBlendFunc( GL_ONE_MINUS_DST_COLOR, GL_ZERO ); //GL CONSTANTS.
	g_theRenderer->DrawLine( crosshairLeft, crosshairRight, Rgba::RED, Rgba::BLUE, HUD_CROSSHAIR_THICKNESS );
	g_theRenderer->DrawLine( crosshairUp, crosshairDown, Rgba::GREEN, Rgba::YELLOW, HUD_CROSSHAIR_THICKNESS );
